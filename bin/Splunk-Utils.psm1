...
# Formats an object into a format easy to interpret by Splunk
function Format-Log {
	[CmdletBinding(ConfirmImpact='Low')]
	param([Parameter(Mandatory=$True)][PSObject]$obj)
	$output = Get-Date -Format "o"
	$obj.PSObject.Properties | ForEach-Object {
		$output += " "
		$output += ($_.Name -replace "\(","_" -replace "\)","" -replace "%","Pct")
		$output += "="
		$output += $_.Value
	}
	return $output
...
Export-ModuleMember -Function 'Reset-Log'
Export-ModuleMember -Function 'Format-Log'